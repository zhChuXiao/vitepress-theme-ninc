 ...
 * @param {function} callback - 回调函数，返回主色调的RGB字符串
 */
 ...
 * 获取图片的主色调并添加透明度
 * @param {string} imageUrl - 图片的URL地址
 * @param {function} callback - 回调函数，返回主色调的RGBA字符串
 * @param {number} alpha - 透明度值（0到1之间）
 * @param {string} method - 获取主色调的方法，可选值为'mostFrequent'或'medianCut'
 */
export const getDominantColorWithAlpha = (imageUrl, callback, alpha = 1, method = 'medianCut') => {
  const img = new Image();
  img.crossOrigin = 'Anonymous'; // 允许跨域加载图片
 ...
 * 获取颜色数组中变化最大的分量索引
 * @param {Array} colors - 颜色数组
 * @returns {number} 变化最大的分量索引
 */
function getComponentIndex(colors) {
 ...
 * @param {string} str - 颜色字符串，可以是RGB或RGBA格式
 ...
 * @param {string} col - 颜色字符串
 * @param {number} amt - 调整量
...
/**
 * 判断颜色是否为浅色
 * @param {string} color - 颜色字符串，可以是RGB或十六进制格式
 * @returns {boolean} 如果是浅色则返回true，否则返回false
 */
...
export const commentText = function (txt) {
