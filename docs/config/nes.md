...
- `XXXX`：4 位 16 进制内存地址
- `Y`：修改类型（0=每帧始终写入、1=只写一次、2=当前值**大于**该值时才写入、3=当前值**小于**该值时才写入）
- `Z`：数值长度声明（当前版本模拟器未消费此位，仅作格式占位）
- `ZZ`：实际数值（16 进制，主题要求恰好 2 位）
...
::: warning 内置马里奥会拼接到用户 roms 末尾
`nes.roms` 为数组，defu 合并规则是 **concat 拼接且不去重**：你配置的 `roms` 在前，内置超级马里奥总会出现在列表末尾，无法仅通过配置移除。若重复配置同 `id` 的游戏，列表中会出现两个同名条目。
:::
...
另外，焦点位于文本输入框（金手指代码、评论框、下拉搜索框等）时，所有功能快捷键均不生效，可放心输入——不必担心输入大写字母时误触 `Shift + Q` 清空存档。
:::
