...
`search` 基于 Algolia InstantSearch（`vue-instantsearch`）渲染，启用后提供全站搜索体验：

- **搜索入口**：导航栏右侧出现搜索按钮，点击后打开「全局搜索」弹窗，弹窗内输入框自动聚焦（`autofocus`）。
- **实时结果列表**：输入关键词后，弹窗内实时展示匹配的文章标题、锚点、摘要与高亮关键词（每页 8 条），点击条目直达对应页面；底部同步显示本次搜索耗时。
- **分页**：结果超过一页时底部展示分页器（`ais-pagination`）；排序由 Algolia 按相关性自动完成，主题未提供手动排序或分类筛选组件。
