...
::: info P2 玩家控制
支持双人游戏。P2 使用方向键移动，数字小键盘的 `1`（B）、`2`（A）、`5`（C 连发）、`4`（D 连发）。连接手柄后可直接使用，无需额外配置。
:::
...
| `Y` | 修改类型（0=每帧始终写入、1=只写一次、2=当前值**大于**该值时才写入、3=当前值**小于**该值时才写入） | `0` |
| `Z` | 数值长度声明（1 或 2 字节；当前版本模拟器未消费此位，仅作格式占位，填 0-4 之间的数字即可） | `1` |
| `ZZ` | 实际数值（16 进制，主题要求恰好 2 位） | `01` |
