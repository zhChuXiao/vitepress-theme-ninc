...
- **当前版本中，文章页面的 JS 文件会原样携带 frontmatter 内容（含明文密码）**，任何访客都可通过浏览器开发者工具直接读取——请勿将其他场合使用的重要密码用作文章密码
...
- **`crypto.password`**：访问密码。在 frontmatter 中写明文，构建时主题会用**站点密钥**和 HMAC-SHA256 算法将其转为哈希值用于解锁验证。注意：文章页面的 JS 文件仍会原样携带 frontmatter 内容（含明文密码），详见上方局限说明——请勿复用重要密码。
- **`articleGPT` 的特殊用法**：加密仅作用于文章正文，frontmatter 中的 `title`、`description`、`cover` 仍会出现在文章列表与 SEO 中。加密文章不填 `articleGPT` 时摘要卡片默认不显示，无需担心卡片泄露；若你手动填写了 `articleGPT`，其内容会**明文**显示在文章页顶部（如 `articleGPT: 当前文档为加密文章不提供摘要`），请勿在其中包含正文关键信息。
