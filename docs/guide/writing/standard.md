...
- **`description` 与 `articleGPT` 的区别**：`description` 用于 SEO meta、文章列表卡片与社交分享卡片，不显示在文章正文区域；`articleGPT` 显示在文章页顶部作为摘要。两者互不回退——未填写 `articleGPT` 时摘要卡片默认不显示（除非开启 [aiSummary](../../config/ai-summary.md) 由主题生成），`description` 也不会出现在文章正文区域。两者都填写能让列表卡片与文章页摘要各司其职。
...
- **`mainColor`**：文章页顶部横幅的背景主色（同时作为封面图与横幅过渡处的内阴影颜色，让封面与横幅自然融合）。未设置时文章页会从封面图自动提取主色调。该字段只作用于文章页，不影响首页列表卡片的配色。值以 `#` 开头时需用单引号包裹，否则 YAML 会将其解析为注释：
