...
导航栏右侧的「开往」按钮，点击后随机跳转至加入 [开往-友链接力](https://www.travellings.cn/) 计划的博客。默认关闭，开启后会在导航栏主题切换按钮与随机文章按钮之间显示开往图标。

详见 [travellings 配置参考](/config/travellings)。

---

### about — 关于本站

「关于本站」页面的内容配置，包含头像技能标签、介绍、追求、技能、项目、座右铭等区域，所有展示内容均来自 `themeConfig.about`，与默认值深合并。

详见 [about 配置参考](/config/about)。
...
构建时调用大模型，为没有手动填写 `articleGPT` 的文章自动生成摘要。默认关闭，开启需要申请 API Key 并配置环境变量。从零接入的完整教程见 [指南 - AI 文章摘要](/guide/ai-summary)。

详见 [aiSummary 配置参考](/config/ai-summary)。
...
内置 NES（任天堂红白机）模拟器页面，支持在浏览器中直接运行经典 NES 游戏，含存档/读档、TAS 录像、键盘与手柄控制。`npx vitepress-theme-ninc init` 生成的项目默认自带超级马里奥 ROM。

详见 [nes 配置参考](/config/nes)。
