...
function updatePath() {
  if (props.containerRef && props.fromRef && props.toRef) {
    ...
    const startX =
      rectA.left - containerRect.left + rectA.width / 2 + (props.startXOffset ?? 0)
    const startY =
      rectA.top - containerRect.top + rectA.height / 2 + (props.startYOffset ?? 0)
    const endX =
      rectB.left - containerRect.left + rectB.width / 2 + (props.endXOffset ?? 0)
    const endY =
